...
mod enabled {
    ...
    fn overlay_hot_fields(old: &ProxyConfig, new: &ProxyConfig) -> ProxyConfig {
        ...
        cfg.general.me_single_endpoint_shadow_writers =
            new.general.me_single_endpoint_shadow_writers;
    ...
    fn warn_non_hot_changes(old: &ProxyConfig, new: &ProxyConfig, non_hot_changed: bool) {
        ...
        if old.censorship.tls_domain != new.censorship.tls_domain
            ...
            || old.censorship.mask_relay_idle_timeout_ms
                != new.censorship.mask_relay_idle_timeout_ms
            || old.censorship.mask_tcp_global_concurrency
                != new.censorship.mask_tcp_global_concurrency
        ...
        if old.general.upstream_connect_retry_attempts
    ...
    fn reload_config(
        ...
        if old_hot.dns_overrides != applied_hot.dns_overrides
            && let Err(e) =
                crate::network::dns_overrides::install_entries(&applied_hot.dns_overrides)
        {
            error!(
    ...
    mod tests {
        ...
        fn reload_recovers_after_parse_error_on_next_attempt() {
            ...
            assert!(
                reload_config(&path, &config_tx, &log_tx, None, None, &mut reload_state).is_none()
            );
            assert_eq!(
                config_tx.borrow().general.ad_tag.as_deref(),
                Some(initial_tag)
...
#[cfg(feature = "hot-reload")]
pub use enabled::spawn_config_watcher;
...
pub fn spawn_config_watcher(
    config_path: PathBuf,
    initial: Arc<ProxyConfig>,
    _detected_ip_v4: Option<IpAddr>,
    _detected_ip_v6: Option<IpAddr>,
) -> (watch::Receiver<Arc<ProxyConfig>>, watch::Receiver<LogLevel>) {
    tracing::info!(
        ?config_path,
        "config hot-reload disabled by build feature; using immutable runtime config"
    );
    let initial_level = initial.general.log_level.clone();
    let (_config_tx, config_rx) = watch::channel(initial);
    let (_log_tx, log_rx) = watch::channel(initial_level);
    (config_rx, log_rx)
}
